...
├── functions                         # Functions folder
    ├── create-customer-function      # Create customer function folder
        ├── src                       # Source code folder
        ├── dist                      # JavaScript files generated by Webpack
        ├── package.json              # No dependency, only required by SAM
    ├── get-customer-by-id-function   # Get customer by ID function folder
        ├── src                       # Source code folder
        ├── dist                      # JavaScript files generated by Webpack
        ├── package.json              # No dependency, only required by SAM
...
├── layers                            # Layers folder
    ├── application-layer             # Application layer folder
        ├── dist                      # JavaScript files of the layer
        ├── package.json              # Local dependencies of the layer, linked to the shared folder
├── shared                            # Shared code folder
    ├── src                           # Source code folder
    ├── dist                          # JavaScript files generated by TypeScript Compiler
    ├── package.json                  # Dependencies for shared
    ├── tsconfig.json                 # TypeScript configuration (extending the one in build folder)
├── openapi                           # OpenAPI folder
    ├── customer.yaml                 # API swagger definition
├── package.json                      # Root package.json file with all scripts needed for build/bundle/deploy
...
This folder contains all the layers. I currently only added one : the application layer.

The layer contains all the dependencies of the shared module.
...
You can add more layer if you need to. But a single application layer should be sufficien in most cases.
...
```
{
  ...
  "dependencies": {
    "@aws-sam-typescript-webpack-backend/shared": "file:../shared"
  },
  "devDependencies": {
...
```
{
  "name": "@aws-sam-typescript-webpack-backend/create-customer-function",
